...
function PLUGIN:Think()
    ...
    if self.next_think <= CurTime() then
        for k, v in next, player.GetAll() do
            ...
            if bac > 0 then
                v:SetNW2Int("lia_alcoholism_bac", math.Clamp(bac - lia.config.get("alcoholism_degraderate", 5), 0, 100))
            end
        end

        self.next_think = CurTime() + lia.config.get("alcoholism_ticktime", 30)
    end
end
...
function PLUGIN:resetBAC(ply)
    ply:SetNW2Int("lia_alcoholism_bac", nil)
end
...
function meta:AddBAC(amt)
    local plugin = lia.plugin.list["alcoholism"]
