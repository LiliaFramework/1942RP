...
function PANEL:Init()
    if IsValid(nut.gui.newCreateCharMenu) then
        nut.gui.newCreateCharMenu:Remove()
    end
    nut.gui.newCreateCharMenu = self
    ...
    do
        ...
        pnl.Setup = function()
            ...
            pnl.faction:SetFont('nutCharButtonFont')
            pnl.faction.Paint = function(me, w, h)
                nut.util.drawBlur(me)
                surface.SetDrawColor(0, 0, 0, 100)
                surface.DrawRect(0, 0, w, h)
            end
            pnl.faction.OnSelect = function(_, _, _, id)
                self.context:SetFaction(nut.faction.teams[id].index)
            end

            local first = true
            for id, _faction in SortedPairsByMemberValue(nut.faction.teams, 'name') do
                if not nut.faction.hasWhitelist(_faction.index) then continue end

                if self.context:GetFaction() then
                    first = false
                    pnl.faction:SetValue(L( nut.faction.get(self.context:GetFaction()).name ))
                end
    ...
    do
        ...
        pnl.Setup = function()
            pnl.modelViewer = pnl:Add('nutModelPanel')
            ...
            function pnl.modelViewer:updateModel()
                local faction = nut.faction.indices[realpanel.context:GetFaction()]
                local modelInfo = faction.models[realpanel.context:GetModel() || 1]
    ...
    do
        ...
        pnl.Setup = function()
            ...
            pnl.finish:SetFont('nutCharButtonFont')
...
local gradient = nut.util.getMaterial('gui/gradient_down')
function PANEL:Paint(w, h)
    ...
    surface.DrawTexturedRect((w / 2) - ((h * 1.5) * .2) / 4 + 2, h - ((h * .2) / 2) * 1.1, ((h * 1.5) * .2) / 2, (h * .2) / 2)
...
function PANEL:Hide()
    ...
    local panel = vgui.Create('nsNewCharacterMenu')
    panel:SetPos(0, -ScrH())
    panel:MoveTo(0, 0, .8)
...
vgui.Register('nsNewCreateCharacterMenu', PANEL, 'EditablePanel')