...
function PANEL:Init()
    if IsValid(nut.gui.newCharsMenu) then
        nut.gui.newCharsMenu:Remove()
    end
    nut.gui.newCharsMenu = self
    self:SetSize(ScrW(), ScrH())
    self:SetPos(0, -ScrH())
    self:MakePopup()
    self:MoveTo(0, 0, .8)

    self.list = self:Add('nutHorizontalScroll')
...
function PANEL:createCharacterSlots()
	...
	for _, id in ipairs(nut.characters) do
		...
		local panel = self.list:Add('nutNewCharacterSlot')
		panel:Dock(LEFT)
...
local gradient = nut.util.getMaterial('gui/gradient_up')
function PANEL:Paint(w, h)
    ...
    surface.DrawTexturedRect((w / 2) - ((h * 1.5) * .2) / 2, 10, (h * 1.5) * .2, h * .2)
...
vgui.Register('nsNewCharactersMenu', PANEL, 'EditablePanel')