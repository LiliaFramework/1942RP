...
function PANEL:isCursorWithinBounds()
	...
	return x >= 0 && x <= self:GetWide() && y >= 0 && y < self:GetTall()
end
...
function PANEL:Init()
	...
	self.name:SetTextColor(nut.gui.newCharsMenu.WHITE)
	...
	self.button.DoClick = function(button)
		nut.gui.newCharsMenu:clickSound()
...
function PANEL:onHoverChanged(isHovered)
	local ANIM_SPEED = nut.gui.newCharsMenu.ANIM_SPEED
	...
	if (isHovered) then
		...
		nut.gui.newCharsMenu:hoverSound()
	else
		self.delete:MoveTo(0, tall, ANIM_SPEED)
	end

	self.faction:AlphaTo(isHovered && 250 || 100, ANIM_SPEED)
end
...
function PANEL:Paint(w, h)
	...
	if (not self:isCursorWithinBounds() && self.isHovered) then
...
vgui.Register('nutNewCharacterSlot', PANEL, 'DPanel')
