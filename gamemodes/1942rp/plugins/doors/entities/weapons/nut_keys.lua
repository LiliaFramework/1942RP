...
function SWEP:PrimaryAttack()
	...
	--]]
    if IsValid(entity) and ((entity:isDoor() and (entity:checkDoorAccess(self.Owner) or (self.Owner:IsAdmin() and self.Owner:Team() == FACTION_staff))) or (entity:IsVehicle() and ((entity.CPPIGetOwner and entity:CPPIGetOwner() == self.Owner) or (entity.GetCreator and entity:GetCreator() == self.Owner)))) then
...
function SWEP:toggleLock(door, state)
    if IsValid(self.Owner) and self.Owner:GetPos():Distance(door:GetPos()) > math.Max(96, door:BoundingRadius() * 1.5) then return end
