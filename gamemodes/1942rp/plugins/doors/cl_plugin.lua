...
function PLUGIN:DrawEntityInfo(entity, alpha)
    if entity.isDoor(entity) and not entity:getNetVar("hidden") and hook.Run("CanDrawDoorInfo", entity) ~= false then
...
netstream.Hook("doorMenu", function(entity, access, door2)
    if IsValid(nut.gui.door) then return nut.gui.door:Remove() end
