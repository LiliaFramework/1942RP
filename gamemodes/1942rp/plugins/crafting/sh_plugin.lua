...
function RECIPES:Register(tbl)
    if not tbl.CanCraft then
        function tbl:CanCraft(player)
            ...
            for k, v in pairs(self.items) do
                ...
                if not inv:getFirstItemOfType(k) then
                    if k == "cube_chip" then
                        local pouch = inv:getFirstItemOfType("cube_chip_pouch")
                        if pouch and (pouch:getData("chipcount", 0) >= v) then continue end
                    end
                ...
                if inv:getFirstItemOfType(k) then
                    if not mult and tonumber(item:getData("Amount")) < v and allStacks(inv, k) < v then
                        return false
                    else
                        mult = false
                    end
    ...
    if not tbl.ProcessCraftItems then
        function tbl:ProcessCraftItems(player)
            ...
            for k, v in pairs(self.items) do
                ...
                if inventory:getFirstItemOfType(k) then
                    --non stack items
                    if itemObj:getData("Amount") == nil then
                    ...
                    --if we're dealing with quantities
                    if itemObj:getData("Amount") ~= nil then
                        --necessary stacks are all in one item
                        if tonumber(itemObj:getData("Amount")) >= v then
...
function RECIPES:CanCraft(player, item)
    local tblRecipe = self:Get(item)
    if not tblRecipe:CanCraft(player) then return 1 end
