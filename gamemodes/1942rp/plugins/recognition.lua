...
do
    ...
    function PLUGIN:IsCharRecognized(char, id)
        ...
        if other then
            ...
            if char:getFaction() == other:getFaction() and not noRecognise[char:getFaction()] and not noRecognise[other:getFaction()] then return end
...
if CLIENT then
    ...
    function PLUGIN:ShouldAllowScoreboardOverride(client)
        if nut.config.get("sbRecog") then return true end
    end
