...
function AdminStick:OpenAdminStickUI(isright, target)
    ...
    if target:IsPlayer() then
        ...
        --[[
        local searchbank = characterInfo:AddOption("Search Item Bank", function()
            RunConsoleCommand("say", "/admincharbanksearch " .. target:SteamID())
            AdminStick.IsOpen = false
        end)
...
net.Receive("namechange", function()
    ...
    net.Receive("namechangeself", function()
    Derma_StringRequest("Change Name", "Pick your new name.", "", function(text)
        net.Start("nameupdate")
        net.WriteString(text)
        net.WriteEntity(LocalPlayer())
        net.SendToServer()
