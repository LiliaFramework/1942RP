...
function PANEL:Init()
    ...
    number.Paint = function(this, w, h)
        draw.SimpleText(self.number, "liaRadioFont", w / 2, h / 2, color_black, 1, 1)
    end
end
...
function PANEL:Init()
    ...
    self.submit.DoClick = function()
        ...
        netstream.Start("radioAdjust", tostring(str), self.itemID)
        self:Close()
    end
    ...
    for i = 1, 5 do
        if i ~= 4 then
            ...
            dot.Paint = function(this, w, h)
                draw.SimpleText(".", "liaRadioFont", w / 2, h - 10, color_black, 1, 4)
            end
        end
    end
end
