...
function META.Sit(ply, EyeTrace, ang, parent, parentbone, func, exit, wantedAng)
    ...
    if SittingOnPlayer:GetBool() then
        ...
        if IsValid(veh) and veh:GetClass() == "prop_vehicle_prisoner_pod" and veh:GetModel() ~= "models/vehicles/prisoner_pod_inner.mdl" and veh:GetDriver() and veh:GetDriver():IsValid() and not veh.PlayerSitOnPlayer then
            ...
            function findSeat(tVeh, depth)
                depth = (depth or 0) + 1
                if IsValid(tVeh:GetParent()) and tVeh:GetParent():GetClass() == "sit_holder" and tVeh:GetParent():GetTargetPlayer() == ply then return true end
                if depth < 50 and IsValid(tVeh:GetParent()) then return findSeat(tVeh:GetParent(), depth) end
        ...
        for k, v in pairs(ents.FindInSphere(EyeTrace.HitPos, 5)) do
            if v.removeonexit then return false end
        end
    end
    ...
    if math.abs(surfaceAng.pitch) <= 15 then
        ...
        if cancelSit then return seat end
...
local function checkAllowSit(ply)
    ...
    if allowSit == false or allowSit == true then return allowSit end
...
for _, v in next, PickupAllowed do
    hook.Add(v, "SA_DontTouchYourself", function(ply, ent)
        if AntiPropSurf:GetBool() and CheckSeat2(ply, ent) == false then return false end
    end)
end

hook.Add("CanTool", "SA_DontTouchYourself", function(ply, tr)
    if AntiToolAbuse:GetBool() and IsValid(tr.Entity) and CheckSeat2(ply, tr.Entity) == false then return false end
end)
...
hook.Add("CanExitVehicle", "Remove_Seat", function(self, ply)
    ...
    checked[self] = true
    local oldpos = self:LocalToWorld(self.oldpos)
