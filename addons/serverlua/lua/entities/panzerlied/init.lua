...
local ent = ents.Create( "panzerlied" )
ent:SetPos( SpawnPos )
ent:Spawn()
ent:Activate()
return ent
...
self.Entity:EmitSound("axis/panzerlied.mp3",500)

for k,ply in pairs(player.GetAll()) do
ply:ChatPrint( "Panzerlied activated")
end
...
self.Entity:StopSound("axis/panzerlied.mp3")

for k,ply in pairs(player.GetAll()) do
ply:ChatPrint( "Panzerlied deactivated")
...
for k,ply in pairs(player.GetAll()) do
ply:ChatPrint( "Panzerlied removed")
ply:SendLua([[RunConsoleCommand("stopsound")]])
end
