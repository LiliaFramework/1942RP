...
-- allows live-update
function addon.EntityFireBullets(attacker, data, ...)
    ...
    function data.Callback(ply, tr, dmgInfo, ...)
        ...
        if seat:IsVehicle() and seat:GetClass() == seat_class then
            ...
            if dmg_ply_veh_seats or not veh then
        ...
        if isfunction(old_Callback) then return old_Callback(ply, tr, dmgInfo, ...) end
    end
end
...
hook.Add("PreGamemodeLoaded", addon_name, function()
    ...
    -- no conflicts by using this instead of a returning hook
    function GAMEMODE:EntityFireBullets(attacker, data, ...)
        ...
        -- always return true to use the callback
